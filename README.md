...
- [1. 空格](#1)
 - [1.1 中英文之间加空格](#1.1)
 - [1.2 中文与数字之间加空格](#1.2)
 - [1.3 数字与英文单位之间加空格、与符号单位之间不加空格](#1.3)
 - [1.4 全角标点与其它字符之间不加空格](#1.4)
 - [1.5 中文与英文半角符号之间加空格](#1.5)
 - [1.6 行的开头和结尾不应出现空白字符](#1.6)
 - [1.7 除特殊场合（如代码中）外，不应有两个或更多空格连续出现](#1.7)
 - [1.8 链接与文字之间加空格（个人习惯）](#1.8)
- [2. 标点、数字、符号](#2)
 - [2.1 使用中文全角标点](#2.1)
 - [2.2 使用直角引号代替弯引号](#2.2)
 - [2.3 使用半角数字](#2.3)
 - [2.4 在用作标题编号时，最高层级的标题编号末尾加小数点和空格（个人习惯）](#2.4)
 - [2.5 使用引号引用英文短语、句子时，引号内使用英文半角标点](#2.5)
 - [2.6 使用正确的省略号和破折号](#2.6)
 - [2.7 正确地使用书名号](#2.7)
 - [2.8 避免叠用叹号、问号和其它不适合叠用的标点](#2.8)
 - [2.9 在表达同时包含疑惑和感叹的语气时，可连用「？！」](#2.9)
 - [2.10 标有引号或书名号的并列成分之间不使用顿号，若有其它成分插在引号或书名号之间则使用顿号（个人习惯）](#2.10)
 - [2.11 标示被注释的词语时，使用方头括号](#2.11)
 - [2.12 注意区分中文数字和阿拉伯数字的使用场景](#2.12)
- [3. 字、词、句](#3)
 - [3.1 部分形近、同义词汇，固定一种写法](#3.1)
 - [3.2 正确地使用「的」「地」「得」](#3.2)
 - [3.3 专有名词使用正确的书写格式](#3.3)
 - [3.4 避免使用无实际意义或指代不清的人称代词](#3.4)
- [4. 内容](#4)
 - [4.1 避免对事物作过于主观的判断或情绪化的质疑](#4.1)
 - [4.2 避免在未经确认的事物之上进行推演](#4.2)
 - [4.3 避免任何带有歧视性的内容](#4.3)
 - [4.4 全文转载需在标题和文章开头注明并给出原文链接，部分引用需在文章末尾以项目列表形式给出引用资料链接](#4.4)
- [5. 参考资料](#5)
